...
module.exports = {
  getCompliment: function (name, numAdjectives = 1, oxfordComma = true) {
    ...
    var adjective = "";
    if (numAdjectives > 1 && numAdjectives < 5) {
      // Return 2-5 adjectives
      ...
      // Get just one
      adjective =
        mutableAdjectives[Math.floor(Math.random() * mutableAdjectives.length)];
    }
    var animal = animals[Math.floor(Math.random() * animals.length)];
    return name + ", you fucking " + adjective + " " + animal + ".";
  },
  getOfficialCompliment: function (name) {
    // Returns an official compliment a la Leslie Knope
    var compliment =
      officialCompliments[
        Math.floor(Math.random() * officialCompliments.length)
      ];
